...
def test_parallel_cache_loading_on_startup(cluster, node_name):
    ...
    keys = node.query(
        "SELECT key FROM system.filesystem_cache WHERE size > 0 ORDER BY key, file_segment_range_begin, size"
    ).strip().splitlines()
    ...
    assert cache_state == node.query(
        f"SELECT key, file_segment_range_begin, size FROM system.filesystem_cache ORDER BY key, file_segment_range_begin, size WHERE key in ({",".join(("'" + x + "'" for x in keys))})"
    )
