...
{
    return compareCursors(lhs, lhs.getRow(), rhs, rhs.getRow(), lhs.sort_columns_size);
}
...
bool ALWAYS_INLINE totallyLess(SortCursorImpl & lhs, SortCursorImpl & rhs)
{
    ...
    int cmp = compareCursors(lhs, lhs.rows - 1, rhs, rhs.getRow(), lhs.sort_columns_size);
    return cmp < 0;
}
    ...
    , log(&Poco::Logger::get("MergeJoinAlgorithm"))
{
    ...
    cursors[0] = createCursor(input_headers[0], on_clause_.key_names_left, strictness);
    cursors[1] = createCursor(input_headers[1], on_clause_.key_names_right, strictness);
}
...
IMergingAlgorithm::Status MergeJoinAlgorithm::merge()
{
    ...
    if (int cmp = totallyCompare(cursors[0]->cursor, cursors[1]->cursor); cmp != 0 && strictness != JoinStrictness::Asof)
    {
        if (cmp < 0)
        {
