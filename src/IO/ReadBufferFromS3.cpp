...
{
    if (auto * session_aware_stream = dynamic_cast<DB::SessionAwareIOStream<DB::HTTPSessionPtr> *>(&read_result.GetBody()))
        return static_cast<DB::HTTPSessionPtr &>(session_aware_stream->getSession());

    /// accept result from S3 mock in gtest_writebuffer_s3.cpp
