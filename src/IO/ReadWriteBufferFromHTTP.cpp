...
{
    LOG_TRACE(log, "getFileSize {}", initial_uri.toString());
                                            ...
                                            bool mute_logging) const
{
    ...
    for (size_t attempt = 0; attempt <= read_settings.http_max_tries; ++attempt)
    {
...
std::unique_ptr<ReadBuffer> ReadWriteBufferFromHTTP::initialize()
{
    ...
    if (response.hasContentLength())
        LOG_TEST(log, "Received response with content length: {}", response.getContentLength());
...
bool ReadWriteBufferFromHTTP::nextImpl()
{
    ...
    doWithRetries(
        /*callable=*/ [&] ()
        {
            ...
            LOG_TEST(log, "nextImpl read_buffer_from_session->next()");

            next_result = impl->next();

            BufferBase::set(impl->buffer().begin(), impl->buffer().size(), impl->offset());

            LOG_TEST(log, "nextImpl read_buffer_from_session->next() ok working_buffer size {} ", working_buffer.size());
...
size_t ReadWriteBufferFromHTTP::readBigAt(char * to, size_t n, size_t offset, const std::function<bool(size_t)> & progress_callback) const
{
    ...
    LOG_TRACE(log, "Sending request to {} for range [{}, {}],"
                   " initial_n {} bytes_copied {} total_bytes_copied {} "
                   "n {} offset {}",
              initial_uri.toString(), offset, offset + n - 1,
              initial_n, bytes_copied, total_bytes_copied,
              n, offset);
    ...
    doWithRetries(
        /*callable=*/ [&] ()
        {
            ...
            LOG_TRACE(log, " try readBigAt Sending request to {} for range [{}, {}],"
                           " initial_n {} bytes_copied {} total_bytes_copied {} "
                           "n {} offset {}",
                      initial_uri.toString(), *range.begin, *range.end,
                      initial_n, bytes_copied, total_bytes_copied,
                      n, offset);
        ...
        /*on_retry=*/ [&] ()
        {
            LOG_TRACE(log, "Try failed readBigAt Sending request to {}, initial_n {} bytes_copied {} total_bytes_copied {} n {} offset {}",
                      initial_uri.toString(), initial_n, bytes_copied, total_bytes_copied, n, offset);
    ...
    LOG_TRACE(log, "Finish readBigAt Sending request to {} initial_n {} bytes_copied {} total_bytes_copied {} n {} offset {}, is_canceled {}",
              initial_uri.toString(), initial_n, bytes_copied, total_bytes_copied, n, offset, is_canceled);
...
{
    LOG_TRACE(log, "seek initial_uri {} offset_ {} ---"
                   "getOffset() {}, range begin {}, offset_from_begin_pos {} --- "
                   "working_buffer.size() {}, getPosition() {}, available() {} --- "
                   "read_buffer_from_session {}, remote_read_min_bytes_for_seek {} ",
              initial_uri.toString(), offset_,
              getOffset(), read_range.begin.value_or(0), offset_from_begin_pos,
              working_buffer.size(), getPosition(), available(),
              bool(impl), read_settings.remote_read_min_bytes_for_seek);
